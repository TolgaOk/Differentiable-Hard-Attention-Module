...
class Dham(Module):
    ...
    def __init__(self, output_size):
        ...
        Parameters:
         ...
         output image's spatial dimensions.
            - conv_seq is a Sequential object which holds conv layers
         of attention module. The last conv layer should have the same
         spatial dimensions as input, and the channel size of that layer
         determines the number of transformed images at the end of the
         module.
        """
        super(Dham, self).__init__()
        self.output_size = output_size
        # self.conv_layers = conv_seq
        self.softlayer = Softmax(-1)
        ...
        self.last_attention_params = None
    ...
    def transform_paramters(self, feature_map, scale_factor=1.0):
        ...
        Parameters:
         ...
         It is expected to have a size of (B, C, Y, X).
            - scale_facor is used to scale the standart deviations.
        """
        ...
        # softmaxed_map = self.softlayer(feature_map.view(B, C, -1)).view(B, C, Y, X)
        softmaxed_map = torch.nn.functional.softmax(feature_map.view(B, C, -1), dim=-1).view(B, C, Y, X)
        # feature_map = torch.sigmoid(feature_map)
        # softmaxed_map = feature_map / feature_map.sum(-1).sum(-1).view(B, C, 1, 1)

        # softmaxed_map = Variable(torch.ones(1, 1, 224, 224).cuda())*1.28e-5
        # softmaxed_map[:, :, 112:, 112:] = torch.ones(1, 1, 112, 112).cuda()*4.16e-5
        # # softmaxed_map = torch.nn.functional.softmax(softmaxed_map.view(B, C, -1), dim=-1).view(B, C, Y, X)
        # plt.imshow(softmaxed_map.data.cpu().numpy()[0, 0], cmap="summer_r")
        # plt.show()
        ...
        norm_x = (grid_x - torch.unsqueeze(mean_x, -1)).detach()
        norm_y = (grid_y - torch.unsqueeze(mean_y, -1)).detach()
        scale = ((norm_x.pow(2).view(B, C, 1, X) + norm_y.pow(2).view(B, C, Y, 1)).sqrt()*softmaxed_map).sum(-1).sum(-1)

        self.last_attention_params = (mean_x, mean_y, scale*scale_factor) 

        #TODO: In place square root and reconsider detach! 
    ...
    def bilinear_transorm(self, input, *raw_ind):
       ...
       <----- 6 ----->
        ...
        Roi = I(YX)*(2/6)*(2/6) + I(Yx)*(4/6)*(2/6) +
            I(yX)*(2/6)*(4/6) + I(yx)*(4/6)*(4/6)
        """
        y_raw_ind, x_raw_ind = raw_ind
        B, C, Y, X = input.size()
        # y_raw_ind.add_(1.).mul_(Y/2)
        # x_raw_ind.add_(1.).mul_(X/2)
        ...
        # plt.imshow(self.gather2d(input, y_u.long(), x_u.long()).data.cpu().numpy()[0, 0, 0], cmap="Greys")
        # plt.show()
    ...
    def _apply(self, fn):
        ...
        return self
    #TODO: Instead of _apply redefine cuda and cpu methods.

if __name__ == "__main__":


    conv_layers = torch.nn.Sequential(
        torch.nn.Conv2d(3, out_channels=1, kernel_size=3, stride=1, padding=1)
    )
    attention_module = Dham((61, 61), conv_layers)
    attention_module.cuda()

    img = Image.open("shoe.png")
    data = np.asarray(img, dtype='uint8').astype(np.float32)
    data = np.expand_dims(np.rollaxis(data/128. -1, 2, 0), 0)

    data = np.zeros((1, 3, 224, 224), dtype=np.float32)
    data[:, :, 112:, 112:] = np.ones((1, 3, 112, 112), dtype=np.float32)
    print(data.shape)

    input = Variable(torch.from_numpy(data).cuda())
    attended_image, feature_map = attention_module(input)
    attended_image = np.squeeze(attended_image.cpu().data.numpy())
    # x, y, s = map(lambda a: a.data.cpu().numpy()[0, 0], params)
    
    # draw_rect(data[0, 1], x, y, s)

    mean_x, mean_y, scale = (attention_module.last_attention_params)
    print(mean_x, mean_y, scale)
    from Mnist_attention import draw_rect
    # plt.imshow(feature_map.data.cpu().numpy()[0, 0], cmap="summer")
    # plt.show()
    B, C, Y , X = feature_map.size()
    soft = torch.nn.functional.softmax(feature_map.view(B, C, -1), dim=-1).view(B, C, Y, X)
    draw_rect(*map(lambda x: x.data.cpu().numpy()[0, 0], (soft, input)), mean_x, mean_y, scale, set_timer=False)

    # plt.imshow(attended_image[1], cmap="summer")
    # plt.show()
